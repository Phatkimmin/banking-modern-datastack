...
def write_to_minio(table_name, records):
    ...
    os.remove(file_path)
    print(f'[SUCCESS] Uploaded {len(records)} records to s3://{bucket}/{s3_key}')
...
print("[SUCCESS] Connected to Kafka. Listening for messages...")
...
for message in consumer:
    ...
    if record:
        # Capture the Debezium transaction timestamp to know which update is newer
        record["_INGESTED_AT"] = payload.get("ts_ms", 0)
